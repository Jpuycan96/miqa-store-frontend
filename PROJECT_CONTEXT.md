...
- Admin/API reemplaza la lista fija de categorías como fuente de verdad. `active` es el estado de publicación: solo categorías activas aparecen en `/api/public/categories`, header, rutas indexables, sitemap y prerender; sus productos también quedan ocultos públicamente al desactivarlas.
- `/productos/:slug` conserva compatibilidad para categorías y productos. Un `canMatch` consulta categorías públicas reales: si el slug pertenece a una categoría carga `Catalog`; en caso contrario continúa a `ProductDetail`. La API impide colisiones entre slugs actuales de ambas clases y aliases históricos. Una alternativa futura más simple sería separar categorías bajo `/categorias/:slug`, pero no se cambió el esquema público aprobado.
- Los slugs de categoría nunca se reciben del Admin: el backend los genera desde `name`, normalizando minúsculas, diacríticos/ñ, separadores y guiones. Al crear siempre calcula el slug; al editar solo lo evalúa si el nombre guardado cambia. Guardar copy editorial o `active` con el mismo nombre conserva slugs legacy como `imprenta-papeleria`. Si un cambio real de nombre produce otro slug, V6 conserva el anterior en `category_slug_aliases`. El Admin muestra solo una vista previa y avisa sobre el cambio de URL. No se agregan sufijos automáticos ante colisiones: la API responde 409.
- `GET /api/public/category-slug-redirects` expone aliases de categorías activas. El build genera `public/_redirects` con reglas 301 para Cloudflare Workers Static Assets. La documentación oficial de Workers confirma que `_redirects` dentro del directorio de assets es parseado por Workers y admite 301; Wrangler 4.131.1 instalado lo excluye del contenido servido, lo convierte en metadata de redirects y su asset handler devuelve `MovedPermanentlyResponse` antes del fallback. La redirección HTTP real existe después de desplegar conjuntamente backend/migración y el siguiente build frontend; Angular no simula un 301. Durante la transición anterior al primer despliegue, un 404 exclusivo de este endpoint produce un archivo vacío porque producción todavía no puede contener aliases V6; otros fallos siguen deteniendo el build.
- Eliminar categoría requiere que no tenga productos; de lo contrario devuelve 409. Al eliminar una vacía, su slug actual e historial quedan reservados como tombstones sin redirección. Con el fallback SPA actual, la URL eliminada todavía no obtiene HTTP 404/410 real: ese pendiente global requiere infraestructura/Worker.
- Sitemap y prerender consultan dinámicamente categorías activas y productos publicados. Flujo: crear/editar/publicar en Admin → API pública/navegación → siguiente build → sitemap, redirects y HTML prerenderizado actualizados. Metadata usa plantilla común `{name} en Trujillo | MIQA`, copy editorial API, canonical propia, OG/Twitter y BreadcrumbList; no existe Product JSON-LD.
