...
@Component({selector:'app-admin-product-form',imports:[ReactiveFormsModule,RouterLink,ProductResources],styleUrl:'./admin.scss',templateUrl:'./product-form.html',changeDetection:ChangeDetectionStrategy.OnPush})
export class AdminProductForm {
 ...
 readonly categories=signal<AdminCategory[]>([]);readonly product=signal<AdminProduct|null>(null);readonly loading=signal(false);readonly busy=signal(false);readonly error=signal('');readonly message=signal('');readonly type=signal<ProductInput['saleType']>('QUANTITY');private manualSlug=!!this.id;
 ...
 save(){if(this.form.invalid||this.busy())return;this.busy.set(true);this.error.set('');const v=this.form.getRawValue();const input:ProductInput={...v,packSize:v.saleType==='PACK'?v.packSize:null,packLabel:v.saleType==='PACK'?v.packLabel:null};
 this.api.saveProduct(this.id,input).pipe(finalize(()=>this.busy.set(false))).subscribe({next:p=>{this.product.set(p);if(p.saleType!=='PACK'){this.form.controls.packSize.setValue(null);this.form.controls.packLabel.setValue('');}this.message.set('Producto guardado.');this.form.markAsPristine();if(!this.id)void this.router.navigate(['/admin/productos',p.id,'editar']);},error:e=>this.error.set(adminError(e))});}
}
