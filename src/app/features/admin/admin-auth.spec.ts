...
describe('Admin authentication',()=>{
 ...
 it('does not send credentials to public API or another origin',()=>{
  ...
  for(const url of [base+'/products',environment.storeApiBaseUrl+'/api/public/products','https://external.example/api/admin/products']){client.get(url).subscribe();const req=http.expectOne(url);expect(req.request.headers.get('Authorization')).toBe(url===base+'/products'?'Bearer test-token':null);req.flush([]);}
 });
